...
public interface BarrageMapper extends MyBaseMapper<Barrage> {
    ...
    @Transactional
    @Insert("insert into yuyue_video_barrage (ID,VIDEO_ID,TEXT,USER_ID,USER_NAME,HEAD_URL,CREATE_TIME)  values  " +
            "(#{id},#{videoId},#{text},#{userId},#{userName},#{headUrl},#{createTime})")
